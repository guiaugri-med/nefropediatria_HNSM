...
def init_db():
    ...
    c = conn.cursor()
    # Cadastro de Paciente e Condutas Iniciais
    ...
    # Registros Diários de Sinais Vitais
...
def gerar_pdf(p, hist):
    ...
    pdf.cell(0, 8, f"Data de Admissão: {p['data_admissao']} | Peso Admissão: {p['peso_seco']} kg", ln=True)
    pdf.cell(0, 8, f"Superfície Corporal: {p['sc']:.2f} m2 | TFGe Inicial: {p['tfge']:.1f}", ln=True)
    pdf.ln(5)
...
with tab1:
    with st.form("cadastro_form"):
        st.subheader("👤 Identificação e Cadastro")
        ...
        # Lógica de K (Schwartz 1 - Colorimétrico)
...
with tab2:
    ...
    if not pacs.empty:
        ...
        with st.form("vitais_form"):
            c1, c2 = st.columns(2)
            d_sv = c1.date_input("Data do Registro")
...
with tab3:
    busca = st.text_input("🔎 BUSCAR PACIENTE PELO NOME").upper()
    if busca:
        conn = sqlite3.connect('nefroped_merces.db')
        ...
        if not p_data.empty:
            ...
            # Botão de Download PDF (Corrigido)
            pdf_bytes = gerar_pdf(p_sel, h_data)
            st.download_button("📥 Baixar Ficha de Monitorização (PDF)", data=pdf_bytes, file_name=f"Ficha_{p_sel['nome']}.pdf", mime="application/pdf")
        else:
